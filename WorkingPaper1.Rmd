...
For each series in the reference set, all candidate models are estimated using the training period, and forecasts are generated for the whole of the test period \textcolor{blue}{(based on "a fixed origin" evaluation)} as set by the competitions. The model corresponding to the smallest MASE [@hyndman2006another] calculated over the test period is selected as the "best model" and forms the *output label* for that series.
