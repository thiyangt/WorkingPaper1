...
To test how well our proposed framework can identify suitable model for forecasting, we use the time series of the M1 [@makridakis1982accuracy] and M3 competitions [@makridakis2000m3]. The R package Mcomp [@hyndmanmcomp] provides the data for both these. The proposed algorithm is applied to yearly, quarterly and monthly series separately.  We run two experiments for each case. In the first experiment, we treat the time series of the M1 competition as the *observed sample* and the time series of the M3 competition as the collection of *new time series* to be forecast. We reverse the sets in the second experiment, using the M3 data as the *observed sample* and the M1 data as the *new time series*. These are summarised in \autoref{tbl:Mcomps}. 
