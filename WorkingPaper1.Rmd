...
\textcolor{blue}{This is a multi-class classification task, with predictors = features, and outcome = categorical variable with all possible forecast models as its class label. In particular, an instance is a tuple $(y_{i1}, y_{i2}, \dots, y_{im}, z_i)$, where predictors = $(y_{i1}, y_{i2}, \dots, y_{im})$ is a vector of $m$ number of features which becomes input to our algorithm and outcome is $z_i$, the "best" forecast model. An example of instance is $(0.51, 0.12, \dots, 3.1, SARIMA)$.}
...
yearly_bp <- ggplot(yearly, aes(x=method, y=MASE, group=method)) + geom_boxplot(aes(fill=method), outlier.size = 0.1) + 
  facet_grid(series ~ h, scales = "free") +theme(axis.text.x = element_text(angle=60, hjust=1))+xlab("")
yearly_bp
```
