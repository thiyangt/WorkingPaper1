...
```{r framework, fig.cap="FFORMS (Feature-based FORecast-Model Selection) framework. The offline phase is shown in blue and the online phase in red."}
...
For each series in the reference set, all candidate models are estimated using the training period, and forecasts are generated for the whole of the test period \textcolor{black}{(based on "a fixed origin" evaluation)} as set by the competitions. The model corresponding to the smallest MASE [@hyndman2006another] calculated over the test period is selected as the "best model" and forms the *output label* for that series.
...
```{r yearlybox, warning=FALSE, message=FALSE, fig.cap="Distribution of MASE values calculated over the new series for yearly data", fig.pos="H", fig.height=5}
library(tidyverse)
...
yearly_bp <- ggplot(yearly_without_wn, aes(x=method, y=MASE, group=method)) + geom_boxplot(aes(fill=method), outlier.size = 0.1)+ facet_grid(series ~ h) +theme(axis.text.x = element_text(angle=60, hjust=1))+xlab("")
yearly_bp
```

```{r quarterlybox, warning=FALSE, message=FALSE, fig.cap="Distribution of MASE values calculated over the new series for quarterly data", fig.pos="H", fig.height=5}
library(tidyverse)
...
quarterly_bp <- ggplot(quarterly_without_wn, aes(x=method, y=MASE, group=method)) + geom_boxplot(aes(fill=method), outlier.size = 0.1) + facet_grid(series ~ h) +theme(axis.text.x = element_text(angle=60, hjust=1))+xlab("")
quarterly_bp
```

```{r monthlybox, warning=FALSE, message=FALSE, fig.cap="Distribution of MASE values calculated over the new series for monthly data", fig.pos="H", fig.height=5}
library(tidyverse)
...
monthly_bp <- ggplot(monthly_without_wn, aes(x=method, y=MASE, group=method)) + geom_boxplot(aes(fill=method), outlier.size = 0.1) + 
