...
studied the applicability of meta-learning approaches for forecas-model
...
non-linearity, the largest Lyapunov exponent, and auto-correlation and
...
forecast-model for a given time series. We define the best
forecast-model as the model with the lowest forecast accuracy measure
(e.g., MAPE or MASE) over the test period.
...
series we need to forecast. For example, if we have only non-seasonal
time series, and no chaotic features, we may wish to restrict our models
to random walks, white noise, ARIMA processes and ETS processes. Even in
this scenario, the number of possible models can be quite large. In
order to identify the best forecast-model for each series, all the
